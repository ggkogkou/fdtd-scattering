...
for t=1 : max_steps
    ...
    % Store current Ez values required by Mur's ABC
    tmp_mur_left = Ez(2, :);
    tmp_mur_right = Ez(cells_x, :);
    tmp_mur_up = Ez(2:cells_x, cells_y);
    tmp_mur_down = Ez(2:cells_x, 2);
    ...
    % Update boundaries using Mur's 1st Order ABC
    Ez(1, :) = tmp_mur_left(1, :) - (dx-c*dt)/(dx+c*dt)*(Ez(2, :)-Ez(1, :)); % Left
    Ez(cells_x+1, :) = tmp_mur_right(1, :) - (dx-c*dt)/(dx+c*dt)*(Ez(cells_x, :)-Ez(cells_x+1, :)); % Right
    Ez(2:cells_x, cells_y+1) = tmp_mur_up(:, 1) - (dx-c*dt)/(dx+c*dt)*(Ez(2:cells_x, cells_y)-Ez(2:cells_x, cells_y+1)); % Up
    Ez(2:cells_x, 1) = tmp_mur_down(:, 1) - (dx-c*dt)/(dx+c*dt)*(Ez(2:cells_x, 2)-Ez(2:cells_x, 1));
